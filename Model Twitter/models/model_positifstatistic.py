...
class model_positifstatistic(osv.osv):
    _name="twitter.positifstatistic"
    _columns={
    ...
    def create(self, cr, uid, values, context=None):
        ...
        if existing:
            super(model_neutrestatistic, self).unlink(cr,uid,existing,context=context)
        ...
        out=super(model_positifstatistic, self).create(cr, uid, data, context=context)
        out=super(model_positifstatistic, self).create(cr, uid, data2, context=context)
        out=super(model_positifstatistic, self).create(cr, uid, data3, context=context)
        out=super(model_positifstatistic, self).create(cr, uid, data4, context=context)
        return out
model_positifstatistic()